...
#
#   Usage:
#        usage: flashbin_to_uf2.py [-h] [-b BASE] [-m MASK] [-o OUTPUT_DIR] [file|glob...]          # noqa: E501
#
...
def get_uf2_romaddr_mask_block( mask_address, romaddr_mask):
    ...
    # and apply the mask offset for each block
    for i in range(16):
        current_block = Block(mask_address + i * 256)
...
def main():
    ...
    parser.add_argument(
        'file', type=str, nargs='+',
        help='List of input ".bin" files or glob patterns'
    )
    parser.add_argument(
        '-b', '--base', dest='base', type=str, default="0x10040000",
        help='Sets the base address where the .bin file is going to be flashed'
    )
    parser.add_argument(
        '-m', '--mask-address', dest='mask', type=str, default="0x1003F000",
        help='Sets the base address where the flashrom address mask will be flashed'
    )
    parser.add_argument(
        '-o', '--output-dir', dest='output_dir', type=str, default=".",
        help='Sets the output directory for the generated files'
    )
    ...
    for pattern in args.file:
        for file in glob.glob(pattern):
            filePath = Path(file)
            outputfilename = output_dir / filePath.with_suffix(".uf2").name
            write_uf2_file(file, outputfilename, baseaddress, maskaddress)
